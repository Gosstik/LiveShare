...
class GoogleOAuthCallbackApiView(PublicApiMixin, APIView):
    ...
    def get(self, request: Request):
        ...
        print(
            f"!!! settings.AUTH_REDIRECT_FRONTEND_URL={settings.AUTH_REDIRECT_FRONTEND_URL}"
        )
    ...
    def _check_greenflow_params(self, params):
        ...
        if "state" not in params:
            ...
            raise BadRequest400(
                code="google_oauth_error",
                detail=(
                        "google oauth error: 'state' is required but not "
                        "presented. You are potentially under CSRF attack!!!"
                ),
            )
            ...
            if response.status_code == 200:
                ...
                if user.profile_icon:
                    old_file_path = os.path.join(
                        settings.MEDIA_ROOT, str(user.profile_icon)
                    )
